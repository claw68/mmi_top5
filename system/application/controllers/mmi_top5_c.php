...
class Mmi_top5_c extends Controller {

	function Mmi_top5_c()
	{
		parent::Controller();
    $this->load->model('Mmi_top5_m','query');
	...
	function index()
	{
	  if($this->session->userdata('valid_top')){
      if($this->session->userdata('judge_id_top')=='101'){
        $data['allJudge'] = $this->query->getAllJudge();
        $data['event'] = $this->query->getAllEvent();
        $this->load->view('mmi_top5_admin',$data);
      }
      else{ 
        $data['judge_name'] = $this->query->getJudgeName($this->session->userdata('judge_id_top'));
        $data['continfo_m'] =   $this->query->getContInfoAll("m");
        $data['continfo_f'] =   $this->query->getContInfoAll("f");
        $data['event'] = $this->query->getAllEvent();
	      $data['countCont'] = $this->query->getCountCont();
		    $this->load->view('mmi_top5_view',$data);
		  }
		}
		else{
    ...
    $account_valid = $this->query->userIdentify($this->input->post('uid'),$this->input->post('password'));
      if($account_valid){
        $this->session->set_userdata('judge_id_top', $account_valid);
        $this->session->set_userdata('valid_top', TRUE);
	...
	{ 
	  ...
	  $judge_id = $this->session->userdata('judge_id_top');
  ...
  function bottom_grid_data()
  {
   ...
   function logout()
  {
   $this->session->set_userdata('valid_top', FALSE);
   echo "<script type=\"text/javascript\">window.location = '../';</script>";
  }
